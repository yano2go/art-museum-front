...
export default function MyCollection(props){
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const response = await axios.get("https://art-museum-back-end.herokuapp.com/api/");
  ...
  useEffect(()=>{
    ...
    const handleClick = async event => {
      event.persist()
      try {
        const response = await axios.get(`https://art-museum-back-end.herokuapp.com/api/${event.target.id}`)
    ...
    const handleDelete = async event => {
      ...
      try {
        await axios.delete(`https://art-museum-back-end.herokuapp.com/api/${event.target.id}`)
        setMyShownPiece({})
      } catch (error) {
        console.error(error)
      }
    ...
    const handleThoughtsSubmit = async event => {
      ...
      const response = await axios.put(`https://art-museum-back-end.herokuapp.com/api/${event.target.id}`, {personalThoughts: event.target.personalThoughts.value})
